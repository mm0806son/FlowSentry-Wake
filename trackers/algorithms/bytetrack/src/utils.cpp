...
vector<STrack>
BYTETracker::sub_stracks(vector<STrack> &tlista, vector<STrack> &tlistb)
{
...
void
BYTETracker::remove_duplicate_stracks(vector<STrack> &resa,
    vector<STrack> &resb, vector<STrack> &stracksa, vector<STrack> &stracksb)
{
...
void
BYTETracker::linear_assignment(vector<vector<float>> &cost_matrix,
...
vector<vector<float>>
BYTETracker::ious(vector<vector<float>> &atlbrs, vector<vector<float>> &btlbrs)
{
...
vector<vector<float>>
BYTETracker::iou_distance(vector<STrack *> &atracks, vector<STrack> &btracks,
...
double
BYTETracker::lapjv(const vector<vector<float>> &cost, vector<int> &rowsol,
    vector<int> &colsol, bool extend_cost, float cost_limit, bool return_cost)
{
  ...
  if (n_rows == n_cols) {
    ...
    if (!extend_cost) {
      ...
      if (ret != 0) {
        std::cerr << "Command failed with return value: " << ret << std::endl;
      }
      exit(0);
    }
  }
