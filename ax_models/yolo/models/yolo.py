...
# Ultralytics YOLOv5 and Darknet YOLOv7 models
#
...
class Detect(nn.Module):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if not torch.onnx.is_in_onnx_export():
                    ...
                    xy = xy * (2.0 * self.stride[i]) + (self.stride[i] * (self.grid[i]))  # new xy
    ...
    def _make_grid(self, nx=20, ny=20, i=0, device=''):
...
class IDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.concat:
            ...
            out = (torch.cat(z, 1), x)
        return x if self.training else out
...
class IKeypoint(nn.Module):
    ...
    def __init__(
        self, nc=80, anchors=(), nkpt=17, ch=(), inplace=True, dw_conv_kpt=False
    ):  # detection layer
...
class Model(nn.Module):
    def __init__(
        ...
        if nc and nc != self.yaml['nc']:
            LOG.debug(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml['nc'] = nc  # override yaml value
        if anchors:
            LOG.debug(f'Overriding model.yaml anchors with anchors={anchors}')
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                ...
                LOG.debug('%10.1f%10.0f%10.1fms %-40s' % (o, m.np, dt[-1], m.type))
...
def parse_model(d, ch):  # meta_map, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        LOG.trace('%3s%18s%3s%10.0f  %-40s%-30s' % (i, f, n, np, t, args))
        save.extend(
