...
ImagingResampleHorizontalConvolution8u4x(
    ...
    int xsize, const int *xbounds, INT16 *kk, int kmax, int coefs_precision)
{
    int xmin, xmax, xx, x;
    INT16 *k;
                            ...
                            const std::vector<double>& prekk)
{
    ...
    int coefs_precision =
        normalize_coeffs_8bpc_original(xsize, ksize, prekk.data(), kk_16);
    int yy = 0;
    for (; yy < ysize - 3; yy += 4) {
        ImagingResampleHorizontalConvolution8u4x(
            (UINT32*)im_out.ptr<cv::Vec4b>(yy),
            (UINT32*)im_out.ptr<cv::Vec4b>(yy + 1),
            (UINT32*)im_out.ptr<cv::Vec4b>(yy + 2),
            (UINT32*)im_out.ptr<cv::Vec4b>(yy + 3),
            (UINT32*)im_in.ptr<cv::Vec4b>(yy),
            (UINT32*)im_in.ptr<cv::Vec4b>(yy + 1),
            (UINT32*)im_in.ptr<cv::Vec4b>(yy + 2),
            (UINT32*)im_in.ptr<cv::Vec4b>(yy + 3), xsize, bounds.data(),
            kk_16.data(), ksize, coefs_precision);
    }
    for (; yy < ysize; yy++) {
        ImagingResampleHorizontalConvolution8u(
            (UINT32*)im_out.ptr<cv::Vec4b>(yy),
            (UINT32*)im_in.ptr<cv::Vec4b>(yy), xsize, bounds.data(),
            kk_16.data(), ksize, coefs_precision);
    }
}