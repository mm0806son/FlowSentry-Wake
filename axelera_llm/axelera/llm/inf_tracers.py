...
from axelera.llm import config, device_manager, logging_utils, utils
import numpy as np

if TYPE_CHECKING:
    from axelera.llm import config
...
class _TritonTrace:
    def __init__(
        ...
        cmd = tt + ["--clear-buffer"] + continuous
        ...
        self._t = threading.Thread(
            target=self._reader_thread, args=(self._p.stdout, self._q), daemon=True
        )
        self._t.start()

    def _reader_thread(self, stream, output_queue):
        try:
            for line in iter(stream.readline, ''):
                output_queue.put(line)
        except Exception as e:
            LOG.warning(f"Error reading from process output: {e}")
        finally:
            stream.close()
    ...
    def stop(self) -> str:
        try:
            ...
            try:
                ...
                LOG.warning("Failed to stop triton_trace process after 2s, killing it")
                self._p.kill()
                self._p.wait()
