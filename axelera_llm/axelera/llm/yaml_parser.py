...
def get_model_details(
    ...
    try:
        axelera_llm = importlib.import_module('axelera.llm')

        if hasattr(axelera_llm, '__path__'):
            pkg_paths = list(axelera_llm.__path__)
            if pkg_paths:
                prefix = str(Path(pkg_paths[0])) + os.sep

    except ImportError:
        env_path = os.path.expandvars("$AXELERA_FRAMEWORK")
        if env_path != "$AXELERA_FRAMEWORK" and env_path:
            prefix = env_path + ("/" if not env_path.endswith("/") else "")

    # Initialize all variables to empty lists to avoid UnboundLocalError
    model_llm_local = []
    model_llm_zoo = []
    model_llm_cards = []

    # Only try to get models if we have a valid prefix
    if prefix and (Path(prefix) / "ax_models").exists():
