...
cpdef eval_cuhk03_cy(float[:,:] distmat, int32_t[:] q_pids, int32_t[:]g_pids,
                     int32_t[:]q_camids, int32_t[:]g_camids, int32_t max_rank):

    cdef int32_t num_q = distmat.shape[0]
    cdef int32_t num_g = distmat.shape[1]
    ...
    cdef:
        int32_t num_repeats = 10
        int32_t[:,:] indices = np.argsort(distmat, axis=1)
        int32_t[:,:] matches = (np.asarray(g_pids)[np.asarray(indices)] == np.asarray(q_pids)[:, np.newaxis]).astype(np.int32)
        ...
        int32_t q_idx, q_pid, q_camid, g_idx
        int32_t[:] order = np.zeros(num_g, dtype=np.int32)
        int32_t keep
        ...
        int32_t num_g_real, num_g_real_masked, rank_idx, rnd_idx
        uint32_t meet_condition
        float AP
        int32_t[:] kept_g_pids, mask
...
cpdef eval_market1501_cy(float[:,:] distmat, int32_t[:] q_pids, int32_t[:]g_pids,
                         int32_t[:]q_camids, int32_t[:]g_camids, int32_t max_rank):
    ...
    cdef:
        int32_t[:,:] indices = np.argsort(distmat, axis=1).astype(np.int32)
        int32_t[:,:] matches = (np.asarray(g_pids)[np.asarray(indices)] == np.asarray(q_pids)[:, np.newaxis]).astype(np.int32)
...
cdef void function_cumsum(cython.numeric[:] src, cython.numeric[:] dst, int32_t n):
    cdef int32_t i
    dst[0] = src[0]
    for i in range(1, n):
        dst[i] = src[i] + dst[i - 1]
